...
func (self *DeviceService) GetByID(id int) (*mngModel.Device, error) {
	device := &mngModel.Device{}
	if err := common.SodaMngDB_R.Where(id).First(device).Error; err != nil {
		return nil, err
	}
	return device, nil
}

func (self *DeviceService) GetBySerialNumber(serialNumber string) (*mngModel.Device, error) {
	device := &mngModel.Device{}
...
func (self *DeviceService) BasicMapBySerialNumber(serialNumber ...string) (map[string]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
	data := make(map[string]*mngModel.Device, 0)
...
func (self *DeviceService) ListBySerialNumber(userId int, schoolId int, serialNumber ...string) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
...
func (self *DeviceService) List(page int, perPage int) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
...
func (self *DeviceService) TotalByUserAndNextLevel(isAssigned bool, user *mngModel.User, serialNumber string, userQuery string, isEqual bool) (int, error) {
	var total int
	params := make([]interface{}, 0)
...
func (self *DeviceService) ListByUserAndNextLevel(isAssigned bool, user *mngModel.User, serialNumber string, userQuery string, isEqual bool, page int, perPage int) (*[]*mngModel.Device, error) {
	list := make([]*mngModel.Device, 0)
	params := make([]interface{}, 0)
	sql := ""
	if userQuery != "" {
...
func (self *DeviceService) ListByUser(userId int, page int, perPage int) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
...
func (self *DeviceService) ListByUserOrderByAddress(userId int, page int, perPage int) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
...
func (self *DeviceService) ListBySerialNumbers(serialNumbers ...string) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
...
func (self *DeviceService) ListByUserAndSerialNumbers(userId int, serialNumbers []string) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
...
func (self *DeviceService) ListByUserAndSchool(userId int, schoolId int, page int, perPage int, serialNums ...string) (*[]*mngModel.Device, error) {
	list := &[]*mngModel.Device{}
	params := make([]interface{}, 0)
...
func (self *DeviceService) Create(device *mngModel.Device) bool {
...
func (self *DeviceService) Update(device mngModel.Device, isBatchUpdate bool) bool {
	var r *gorm.DB
	transAction := common.SodaMngDB_WR.Begin()
	...
	if isBatchUpdate {
		...
		r = transAction.Model(&mngModel.Device{}).Where("serial_number in (?)", serialNumber).Updates(deviceMap).Scan(&device)
	} else {
		r = transAction.Model(&mngModel.Device{}).Where("id = ?", device.ID).Updates(deviceMap).Scan(&device)
	}
	if r.Error != nil {
...
func (self *DeviceService) UpdatePulseName(device mngModel.Device) bool {
	transAction := common.SodaMngDB_WR.Begin()
	r := transAction.Model(&mngModel.Device{}).Where("id = ?", device.ID).Updates(device)
	if r.Error != nil {
...
func (self *DeviceService) UpdateStatus(device *mngModel.Device) (*mngModel.Device, error) {
	if err := common.SodaMngDB_WR.Model(&mngModel.Device{}).Where(device.ID).Update("status", device.Status).Scan(device).Error; err != nil {
		return nil, err
	}
	return device, nil
}

func (self *DeviceService) UpdateBySerialNumber(device *mngModel.Device) bool {
	transAction := common.SodaMngDB_WR.Begin()
	r := transAction.Model(&mngModel.Device{}).Where("serial_number = ?", device.SerialNumber).Updates(device)
	if r.Error != nil {
	...
	r = transAction.Model(&mngModel.Device{}).Where("serial_number = ?", device.SerialNumber).Update("school_id", device.SchoolID).Scan(device)
	if r.Error != nil {
		common.Logger.Warningln("DB Update BoxInfo-BY-DEVICENO:", r.Error.Error())
		transAction.Rollback()
		return false
...
func (self *DeviceService) BatchUpdateBySerialNumber(device *mngModel.Device, serialList []string) bool {
	transAction := common.SodaMngDB_WR.Begin()
	device.SerialNumber = "" //不更新这个字段
	r := transAction.Model(&mngModel.Device{}).Where("serial_number IN (?)", serialList).Updates(device)
	if r.Error != nil {
		common.Logger.Warningln("DB Update BoxInfo-BY-DEVICENO:", r.Error.Error())
		transAction.Rollback()
		return false
	...
	r = transAction.Model(&mngModel.Device{}).Where("serial_number IN (?)", serialList).Updates(zeroValue).First(device)
	if r.Error != nil {
		common.Logger.Warningln("DB Update BoxInfo-BY-DEVICENO:", r.Error.Error())
		transAction.Rollback()
		return false
...
func (self *DeviceService) Reset(id int, user *mngModel.User) (*mngModel.Device, error) {
	device := &mngModel.Device{}
	transAction := common.SodaMngDB_WR.Begin()
	...
	if err := transAction.Model(&mngModel.Device{}).Where("id = ?", id).Updates(data).Scan(device).Error; err != nil {
		transAction.Rollback()
		return nil, err
	}
	transAction.Commit()
...
func (self *DeviceService) ResetPasswordStep(serialNumber string, step int) error {
	...
	r := transAction.Model(&mngModel.Device{}).Where("serial_number = ?", serialNumber).Updates(data)
	if r.Error != nil {
		transAction.Rollback()
		return r.Error
	}
...
func (self *DeviceService) Delete(id int) bool {
	...
	transAction := common.SodaMngDB_WR.Begin()
	//硬删除记录
	r := transAction.Model(&mngModel.Device{}).Where("id = ?", id).Scan(device).Unscoped().Delete(&mngModel.Device{})
	if r.Error != nil {
...
func (self *DeviceService) TotalByUserIds(userIds []int) (int, error) {
	//list := &[]*model.Device{}
	var total int64
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("user_id IN (?)", userIds).Count(&total)
	if r.Error != nil {
		return 0, r.Error
	}
...
func (self *DeviceService) IsOwntoMeOrTest(userId int, serialList []string) bool {
	var total int64
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("user_id IN (1,?) AND serial_number IN (?)", userId, serialList).Count(&total)
	if r.Error != nil {
...
func (self *DeviceService) IsNoExist(serialList []string) bool {
	var total int64
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("serial_number IN (?)", serialList).Count(&total)
	if r.Error != nil {
		common.Logger.Warningln(r.Error)
		return false
	}
...
func (self *DeviceService) BatchCreateBySerialNum(device *mngModel.Device, serialList []string) error {
	transAction := common.SodaMngDB_WR.Begin()
...
func (self *DeviceService) ListByUserIds(userIds []int) (*[]*mngModel.Device, error) {
	list := []*mngModel.Device{}
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("user_id in (?)", userIds).Find(&list)
	if r.Error != nil {
		return nil, r.Error
	}
	return &list, nil
...
func (self *DeviceService) ListByUserId(userId int) (*[]*mngModel.Device, error) {
	list := []*mngModel.Device{}
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("user_id = ?", userId).Find(&list)
	if r.Error != nil {
		return nil, r.Error
	}
	return &list, nil
...
func (self *DeviceService) BasicByUserId(userId int) (*mngModel.Device, error) {
	device := &mngModel.Device{}
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("user_id = ?", userId).First(device)
	if r.Error != nil {
		return nil, r.Error
	}
	return device, nil
...
func (self *DeviceService) BasicMapByUserId(userId ...int) (map[string]*mngModel.Device, error) {
	list := []*mngModel.Device{}
	deviceMap := make(map[string]*mngModel.Device, 0)
	r := common.SodaMngDB_R.Model(&mngModel.Device{}).Where("user_id in (?)", userId).Find(&list)
	if r.Error != nil {
		return nil, r.Error
	}
	for _, _device := range list {
...
func (self *DeviceService) Assign(toUser *mngModel.User, fromUser *mngModel.User, serialNumbers []string) (bool, error) {
	transAction := common.SodaMngDB_WR.Begin()
	...
	r := transAction.Model(&mngModel.Device{}).Where("serial_number in  (?) ", serialNumbers).Updates(data)
	if r.Error != nil {
		transAction.Rollback()
...
func timedUpdateStatus(status string, timedDuration time.Duration) (bool, error) {
	...
	r := common.SodaMngDB_WR.Model(&mngModel.Device{}).Where("status = " + status + " and updated_at <= ?", time.Now().Add(timedDuration).Format(timeFormat)).Update("status", 0)
	if r.Error != nil {
		common.Logger.Warningln("Timed Update " + status + " Device-STATUS:", r.Error.Error())
		return false, r.Error
	}
...
func (self *DeviceService) Paging(userIDs []int, fromUserIDs []int, label string, serialNumber string, referDeviceID int, schoolID int, statuses []int, offset int, limit int) (*entity.PaginationData, error) {
	pagination := entity.PaginationData{}
	deviceList := make([]*mngModel.Device, 0)
	...
	if label != "" {
		scopes = append(scopes, func(db *gorm.DB) *gorm.DB {
			return db.Where("label like (?)", "%" + label + "%")
		})
	}
	...
	if referDeviceID != 0 {
		scopes = append(scopes, func(db *gorm.DB) *gorm.DB {
			return db.Where("reference_device_id = ?", referDeviceID)
		})
	}

	if len(fromUserIDs) != 0 {
		scopes = append(scopes, func(db *gorm.DB) *gorm.DB {
			return db.Where("from_user_id in (?)", fromUserIDs)
		})
	}
	...
	if err := db.Model(&mngModel.Device{}).Scopes(scopes...).Count(&pagination.Pagination.Total).Offset(offset).Limit(limit).Order("id desc").Find(&deviceList).Error; err != nil {
		return nil, err
	}
	...
	pagination.Objects = deviceList
	return &pagination, nil
}

func (self *DeviceService) GetAllReferenceDevice() (*[]*mngModel.ReferenceDevice, error) {
	referenceDeviceList := []*mngModel.ReferenceDevice{}

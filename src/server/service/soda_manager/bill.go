...
func (self *BillService) userIdByUserCondition(sql string, value []interface{}) ([]int, error) {
	ids := make([]int, 0)
	var id int
	r, err := common.SodaMngDB_R.Model(mngModel.User{}).Select("id").Where(sql, value...).Rows()
	if err != nil {
		return ids, err
	}
	defer r.Close()
...
func (self *BillService) ListByAccountTypeAndTimeType(accountType, status, dateType, offset, limit int, startAt, endAt, keys string) ([]*mngModel.Bill, error) {
	billList := []*mngModel.Bill{}
...
func (self *BillService) ListByBillIdsAndStatus(billIds []interface{}, status []interface{}) ([]*mngModel.Bill, error) {
	billList := []*mngModel.Bill{}
...
func (self *BillService) BatchUpdateStatusById(status int, ids []interface{}) error {
	...
	param["status"] = status
	r := tx.Model(&mngModel.Bill{}).Where(" bill_id in (?) ", ids).Update(param)
	if r.Error != nil {
		tx.Rollback()
		return r.Error
	}
	// 接着更新daily_bill
	r = tx.Model(&mngModel.DailyBill{}).Where(" bill_id in (?) ", ids).Update(param)
	if r.Error != nil {
		tx.Rollback()
		return r.Error
	}
...
func (self *BillService) BatchUpdateSubmitAtById(status int, ids []interface{}) error {
	...
	r = tx.Model(&mngModel.DailyBill{}).Where(" bill_id in (?) ", ids).Update(dailyBillParam)
	if r.Error != nil {
		tx.Rollback()
		return r.Error
	}
...
func (self *BillService) GetFirstWechatBill() (*mngModel.Bill, error) {
	bill := &mngModel.Bill{}
...
func (self *BillService) Updates(list *[]*mngModel.Bill) error {
	tx := common.SodaMngDB_WR.Begin()
	...
	r := tx.Model(&mngModel.Bill{}).Where(" bill_id in (?) and status != 2 ", successBillIDs).Updates(map[string]interface{}{
		"status":     2,
		"settled_at": time.Now(),
	})
	if r.Error != nil {
	...
	r = tx.Model(&mngModel.Bill{}).Where(" bill_id in (?) and status != 2 ", failBillIDs).Updates(map[string]interface{}{
		"status":     4,
		"settled_at": time.Now(),
	})
	if r.Error != nil {
	...
	r = tx.Model(&mngModel.DailyBill{}).Where(" bill_id in (?) and status != 2 ", successBillIDs).Updates(map[string]interface{}{
		"status":     2,
		"settled_at": time.Now(),
	})
	if r.Error != nil {
	...
	r = tx.Model(&mngModel.DailyBill{}).Where(" bill_id in (?) and status != 2 ", failBillIDs).Updates(map[string]interface{}{
		"status":     4,
		"settled_at": time.Now(),
	})
	if r.Error != nil {
...
func (self *BillService) BasicById(id int) (*mngModel.Bill, error) {
	bill := &mngModel.Bill{}
...
func (self *BillService) BasicByBillId(billId string) (*mngModel.Bill, error) {
	bill := &mngModel.Bill{}
...
func (self *BillService) ReportMapByPeriodAndAccountType(start, end string, accountType int) (*map[string]map[string]interface{}, error) {
	...
	sql := "select sum(cast) Cast,sum(total_amount) TotalAmount,date(settled_at) SettledAt from bill where " +
		"date(settled_at) >= ? and date(settled_at) <= ? and account_type = ? and status = 2 " + // 必须要获取到成功的账单

...
func (self *SchoolService)GetByID(id int) (*mngModel.School, error) {
...
func (self *SchoolService)Paging(name string, provinceID string, cityID string, areaID string, offset int, limit int) (*entity.PaginationData, error) {
	...
	schoolList := make([]*mngModel.School, 0)
	...
	if err := db.Model(&mngModel.School{}).Scopes(scopes...).Count(&pagination.Pagination.Total).Offset(offset).Limit(limit).Order("id desc").Find(&schoolList).Error; err != nil {
		return nil, err
	}
...
func (self *SchoolService)Create(school *mngModel.School) (*mngModel.School, error) {
...
func (self *SchoolService)Update(school *mngModel.School) (*mngModel.School, error) {
	...
	if err := common.SodaMngDB_WR.Model(&mngModel.School{}).Where(school.ID).Updates(_school).Scan(school).Error; err != nil {
		return nil, err
	}
	return school, nil
}
...
func (self *SchoolService)Delete(id int) error {
	...
	if err := tx.Unscoped().Delete(&mngModel.School{}, id).Error; err != nil {
		tx.Rollback()
		return err
	}
	tx.Commit()
